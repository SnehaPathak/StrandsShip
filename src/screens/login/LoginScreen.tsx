  ...
  const onSubmit: SubmitHandler<FormValues> = (data) => {
    if (!formError) {
      ...
      try {
        ...
        //   });
        AuthManager.DoLogin(email.current, password.current)
          .then(response => {
            setIsSubmitting(false);
            if (response.success) {
              console.log(`response user is ${JSON.stringify(response.item)}`)
              navigation.reset({
                index: 0,
              ...
              submissionError.current = response.message;
              // setSubmitError(response.message);
              console.log(`submission error is ${JSON.stringify(response)}`)
            }
          })
  ...
  return (
    <View style={LoginCommonStyle.container}>
      ...
      <View style={LoginCommonStyle.containerOverlay}>
        <Text style={LoginStyle.errorMessage}>{submitError}</Text>
        ...
        <View style={LoginStyle.buttonContainer}>
          {isSubmitting && <ActivityIndicator></ActivityIndicator>}
          <ButtonCommon
            title="Submit"
            theme="orange"
