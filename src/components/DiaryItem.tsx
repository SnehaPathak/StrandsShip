...
import { ICustomerNoteViewModel, NoteKindViewModel } from '../types/ViewModels/Index';

const TITLE_ICON_SIZE = 16;

const DiaryItem = (itemInfo: ICustomerNoteViewModel) => {
    ...
    if (itemInfo) {
        ...
        if (itemInfo.note_kind) {

            if (itemInfo.note_kind == NoteKindViewModel.Note.toString()) {
                titleIconContent = (
        ...
        if (itemInfo.stamp_utc) {
            titleContent = (
                <Text style={DiaryItemStyles.titleLabel}>{itemInfo.stamp_utc}</Text>
            )
        }
        ...
        if (itemInfo.text) {
            textContent = <Text style={DiaryItemStyles.text}>{itemInfo.text}</Text>
        }
        ...
        content = (
            <Surface style={false ? DiaryItemStyles.containerWithImage : DiaryItemStyles.containerWithoutImage} elevation={0}>
